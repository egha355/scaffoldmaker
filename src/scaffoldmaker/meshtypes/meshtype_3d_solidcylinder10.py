...
class MeshType_3d_solidcylinder10(Scaffold_base):
    '''
    classdocs
    '''
    @staticmethod
    def getName():
        return '3D Solid Cylinder 10'
    ...
    def generateBaseMesh(cls, region, options):
        ...
        # The other half
        tShield2 = ShieldMesh( elementsCountAcross, elementsCountUp, elementsCountRim, None, elementsCountAlong)


        if tShield2.elementsCountThroughWall>1:
            for n2 in range(tShield2.elementsCountUp):
                for n3 in range(tShield2.elementsCountThroughWall + 1):
                    for n1 in range(tShield2.elementsCountAcross + 1):
                        if tShield.px[0][n2][n1]:
                            tShield2.px[n3][n2][n1] = mirror.mirrorImageOfPoint(tShield.px[n3][n2][n1],[0.0, 0.0, 1.0, 0.0])
                            tShield2.pd1[n3][n2][n1] = mirror.mirrorVector(tShield.pd1[n3][n2][n1],[0.0, 0.0, 1.0, 0.0])
                            tShield2.pd2[n3][n2][n1] = mirror.mirrorVector(tShield.pd2[n3][n2][n1],[0.0, 0.0, 1.0, 0.0])
                            tShield2.pd3[n3][n2][n1] = mirror.mirrorVector(tShield.pd3[n3][n2][n1],[0.0, 0.0, 1.0, 0.0])
        ...
        # create the other half nodes excluding the nodes on the mirror plane
        nodeIdentifier = tShield2.generateNodes(fm, coordinates, nodeIdentifier)

        # assign the nodes on the mirror plane for creating elements on the other half
        for n2 in [tShield2.elementsCountUp]:
            for n3 in range(tShield2.elementsCountThroughWall + 1):
                for n1 in range(tShield2.elementsCountAcross + 1):
                    if tShield.px[0][n2][n1]:
                        tShield2.px[n3][n2][n1] = tShield.px[n3][n2][n1]
                        tShield2.pd1[n3][n2][n1] = tShield.pd1[n3][n2][n1]
                        tShield2.pd2[n3][n2][n1] = tShield.pd2[n3][n2][n1]
                        tShield2.pd3[n3][n2][n1] = tShield.pd3[n3][n2][n1]
                        tShield2.nodeId[n3][n2][n1] = tShield.nodeId[n3][n2][n1]
        ...
        # create the other half nodes excluding the nodes on the mirror plane
        tShield2.elementsCountUp -= 1
        elementIdentifier = tShield2.generateElements(fm, coordinates, elementIdentifier, [btMeshGroup])

        # create the last layer elements attached to the other half
        tricubichermite = eftfactory_tricubichermite(mesh, useCrossDerivatives)
        eft = tricubichermite.createEftNoCrossDerivatives()
        elementtemplate = mesh.createElementtemplate()
        elementtemplate.setElementShapeType(Element.SHAPE_TYPE_CUBE)
        for e3 in range(tShield2.elementsCountThroughWall):
            for e2 in [tShield2.elementsCountUp]:
                for e1 in range(tShield2.elementsCountAcross):
                    setEftScaleFactorIds(eft, [1], [])
                    scalefactors = [-1.0]
                    nids = [tShield2.nodeId[e3][e2][e1], tShield2.nodeId[e3][e2][e1 + 1], tShield2.nodeId[e3][e2 + 1][e1], tShield2.nodeId[e3][e2 + 1][e1 + 1],
                            tShield2.nodeId[e3 + 1][e2][e1], tShield2.nodeId[e3 + 1][e2][e1 + 1], tShield2.nodeId[e3 + 1][e2 + 1][e1], tShield2.nodeId[e3 + 1][e2 + 1][e1 + 1]]
                    remapEftNodeValueLabel(eft, [3, 4, 7, 8], Node.VALUE_LABEL_D_DS2, [(Node.VALUE_LABEL_D_DS2, [1])])
                    elementtemplate.defineField(coordinates, -1, eft)
                    element = mesh.createElement(elementIdentifier, elementtemplate)
                    result2 = element.setNodesByIdentifier(eft, nids)
                    elementIdentifier+=1
                    tShield2.elementId[e2][e1] = elementIdentifier
                    elementIdentifier += 1
